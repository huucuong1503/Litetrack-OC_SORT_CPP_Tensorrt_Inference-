...
namespace ocsort {
    ...
    std::vector<Eigen::RowVectorXf> OCSort::update(Eigen::MatrixXf dets) {
        ...
        for (int i = 0; i < trks.rows(); i++) {
            ...
            posX=pos(0); posY=pos(1); posW=pos(2)-pos(0); posH=pos(3)-pos(1);
            std::cout<<"posx: "<<posX<<" posy: "<<posY<<"  posW: "<<posW<<"  posH: "<<posH<<std::endl;
        ...
        for (auto m : unmatched_trks) {
                // Print the estimated state before the update
            // std::cout << "Detector lost: Tracker ID " << trackers[m].id 
            //         << ", Estimated state: " << trackers[m].get_state() 
            //         << std::endl;
        ...
        for (int i : unmatched_dets) {
            ...
            // std::cout<<"Detector losssssss"<<std::endl;
        }
        ...
        for (int i = trackers.size() - 1; i >= 0; i--) {
            ...
            // std::cout<<"x:  "<<d[0]<<"  y:  "<<d[1]<<"  w:  "<<d[2]-d[0]<<"  h:  "<<d[3]-d[1]<<std::endl;
